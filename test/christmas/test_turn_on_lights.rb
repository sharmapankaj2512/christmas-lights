...
module Christmas
  class TestTurnLightsOn < Minitest::Test
    def test_lights_on
      ...
      grid.turn_on(TwoDimensionalRange.new([0, 0], [0, 0]))
    ...
    def test_light_on_two_by_two
      ...
      grid.turn_on(TwoDimensionalRange.new([0, 1], [0, 1]))
    ...
    def test_lights_on_two_by_two_horizontal_line
      ...
      grid.turn_on(TwoDimensionalRange.new([0, 0], [1, 0]))
    ...
    def test_lights_on_two_by_two_vertical_line
      grid = Grid.new(2)
      grid.turn_off(TwoDimensionalRange.new([0, 0], [1, 1]))
      grid.turn_on(TwoDimensionalRange.new([0, 0], [0, 1]))
    ...
    def test_lights_on_two_by_two_square
      grid = Grid.new(2)
      grid.turn_off(TwoDimensionalRange.new([0, 0], [1, 1]))
      grid.turn_on(TwoDimensionalRange.new([0, 0], [1, 1]))
